    ...
    while((dp = readdir(dir)) != NULL){
        if(Is_sub(dp->d_name, ".", 1) != 0){
            for(int i = 0; i < 2 * depth; i++){
                if(i % 2 == 0)  printf("%c", 179);   //179 is |
                else    printf(" ");
            }
            printf("%c%c%s\n", 195, 196, dp->d_name);   //195 is |- and 196 is --
    ...
    if(validate(command1) == 1 && validate(command2) == 1 && arman == 1){
        ...
        //find command
            ...
            //else if there are properties
                ...
                prop1[k] = '\0';
                if((prop1[0] == 'a' && prop1[1] == 't' && prop1[2] == '\0')/* prop1 == "at" */ || (prop1[0] == 'b' && prop1[1] == 'y' && prop1[2] == 'w' && prop1[3] == 'o' && prop1[4] == 'r' && prop1[5] == 'd' && prop1[6] == ' ' && prop1[7] == '-' && prop1[8] == 'a' && prop1[9] == 't')){
                    int o = 0;
...
//            pos1[k] = '\0';
            k++;
            int l = 0;
            while(command2[l + i + k] != '\0' && command2[l + i + k] != NULL && command2[l + i + k] != '\n'){
...
/*
        ...
        char command3[] = "compare nil.txt ko.txt";
        char command4[] = "insertstr --file fuf.txt -pos 30:0";

        char input1[1000];
        if(Is_sub(command3, "compare ", 8) == 0){
            char path1[100];
            char path2[100];
            int i = 0;
            if(command3[8] != '"'){
                while(command3[i + 8] != ' '){
                    path1[i] = command3[i + 8];
                    i++;
                }
                path1[i] = '\0';
                i++;
                int j = 0;
                while(!(command3[i + 8 + j] == ' ' && command3[i + 9 + j] == '=' && command3[i + 10 + j] == 'D' && command3[i + 11 + j] == ' ') && command3[i + 8 + j] != '\0'){
                    path2[j] = command3[i + j + 8];
                    j++;
                }
                path2[j] = '\0';
            }
...
//            comparator_arman(path1, path2, input);
    ...
    if(!exist(path1))    strcpy(input1, "file1 is not found");
    else if(!exist(path2))    strcpy(input1, "file2 is not found");
    else{
        FILE *fff1;
        fff1 = fopen(path1, "r");
        FILE *fff2;
        ...
        fclose(fff2);
            ...
            if(Is_sub(command4, "insertstr --file ", 17) == 0){
            char path[100];
            char str[100];
            char pos1[100];
            char pos2[100];
            ...
            k++;
            int l = 0;
            while(command4[l + i + k] != '\0' && command4[l + i + k] != NULL && command4[l + i + k] != '\n'){
                pos2[l] = command4[l + i + k];
                l++;
            }
            int x = atoi(pos1);
            int y = atoi(pos2);
